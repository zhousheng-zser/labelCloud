...
def surface_equ_3d_jit(polygon_surfaces):
    ...
    d = np.sum(normal_vec * polygon_surfaces[:, :, 0, :], axis=-1)  # 鏄惧紡骞挎挱涔樻硶+姹傚拰
    return normal_vec, -d
...
def compute_3D_line_v2(x1,x2, y1,y2, z1,z2, color, d):
    ...
    save_points = np.zeros((7,d), dtype = float)
...
def compute_line(x1, x2, d=300):
    if x1 == x2:
        return np.ones((d,), dtype=float) * x1
    elif x1 < x2:
        return np.linspace(x1, x2, d)
    elif x1 > x2:
        return np.linspace(x1, x2, d)
...
def draw_circle(center, r, axis=2, color=[0, 0, 255], res=0.01):
    ...
    x = r * np.cos(theta)
    y = r * np.sin(theta)
    save_circle = np.zeros((7, x.shape[0]), dtype=float)
    if axis == 0:
        x = center[1] + x
        y = center[2] + y
